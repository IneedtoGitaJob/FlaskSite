...
def getTwitterPositivity(searchRequest, Years):
    ...
    for year in range(earliestYear, currentYear, 1):
        sentimentTotalForEachYear.append(getYearPositivityTwitter(searchRequest,year,sia))
...
def getYearPositivityTwitter(searchRequest,year,sia):
        data = scrape(
        words=[searchRequest], since= str(year)+"-01-01", until=str(year)+"-12-31", from_account=None, interval=80,
        headless=True,limit = 4, display_type="Top", save_images=False, lang="en",
