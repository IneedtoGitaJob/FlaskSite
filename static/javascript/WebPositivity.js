      ...
      if (sentimentTotalForEachYear.length > 0) {
          ...
          new Chart(canvas, {
              ...
              options: {
                ...
                scales:{yAxes: [{ticks: {min: -1, max:1}}]},
                  maintainAspectRatio: false,
                  title: {
                      display: true,
  ...
  if (TextInput == "") {
      ...
      switch (RadioInput) {
          ...
          case "Website":
              ...
              break;
          case "Tweets":
              if(VerifyYears(yearsInput)){checkTwitter(TextInput, yearsInput);}
              else{alert("Invalid years input must be an integer that represents the number of years back from the current year you would like to check");}
              break;
          case "News":
            if(VerifyYears(yearsInput)){checkManyWebs(TextInput, yearsInput);}
              else{alert("Invalid years input must be an integer that represents the number of years back from the current year you would like to check");}
              break;
      }
  }
